...
def local_event(pid, clock):
    clock[pid-1] += 1
    print('Process {} performed local event. Vector timestamp is {}'.format(pid, clock))
    return clock
...
def send_event(pipe, pid, clock):
    clock[pid-1] += 1
    pipe.send((pid, clock))
    print('Process {} sent message. vector timestamp is {}'.format(pid, clock))
    return clock
...
def receive_event(pipe, pid, clock):
    ...
    ts[pid-1] = max(ts[pid-1], clock[pid-1]) + 1

    print('Process {} received message from Process {}. vector timestamp is {}'.format(pid, sender_id, ts))
    return ts
